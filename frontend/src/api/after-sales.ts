...
export interface AfterSalesOrderLine { salesOrderItemId:number; skuId:number; productCode?:string; customerPartNumber?:string; productName?:string; model?:string; configuration?:string; unit?:string; shippedQuantity:number; availableReturnQuantity:number; salePrice?:number }
export interface AfterSalesReturnLine extends AfterSalesOrderLine { id?:number; requestedQuantity:number; returnUnitPrice?:number; receivedQuantity?:number; goodQuantity?:number; defectiveQuantity?:number }
...
export interface AfterSalesCommand { orderId:number; applicationDate:string; issueDescription:string; contactName?:string; contactPhone?:string; deliveryAddress?:string; remark?:string; version?:number; returnLines:{salesOrderItemId:number;requestedQuantity:number;returnUnitPrice:number}[]; replacementLines:{salesOrderItemId?:number;returnLineId?:number;skuId:number;plannedQuantity:number}[] }
