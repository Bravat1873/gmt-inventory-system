...
import { ref } from 'vue'; import { login } from '../api/auth'
const emit=defineEmits<{loggedIn:[user:{id:number;username:string;displayName:string}]}>(); const username=ref('admin'); const password=ref(''); const error=ref(''); const busy=ref(false)
