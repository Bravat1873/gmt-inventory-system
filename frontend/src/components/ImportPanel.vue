    ...
    <div class="simple-import-body">
      ...
      <section v-else-if="previewBatch && importedRows === null && type === 'ORDER'" class="order-preview" aria-label="订单导入预览">
        ...
        <section v-for="group in orderGroups" :key="group.key" class="order-preview-group" :data-test="`order-preview-group-${group.key}`">
          ...
          <div class="supplier-preview-table-wrap"><table class="order-preview-table"><thead><tr><th>产品编号</th><th>客户料号</th><th>型号</th><th>数量</th><th>含税单价</th><th>明细备注</th><th>状态</th><th>错误</th></tr></thead><tbody><tr v-for="row in group.rows" :key="row.id" data-test="order-preview-row" :class="{ 'conflict-row': row.data._conflict }"><td>{{ value(row, 'productCode') }}</td><td>{{ value(row, 'customerPartNumber') }}</td><td>{{ value(row, 'model') }}</td><td>{{ value(row, 'quantity') }}</td><td>{{ value(row, 'salePrice') }}</td><td class="order-preview-remark" data-test="order-preview-remark">{{ value(row, 'remark') }}</td><td>{{ row.status === 'VALID' ? '有效' : row.status === 'ERROR' ? '错误' : '忽略' }}</td><td>{{ orderGroupError(group.key) ? '—' : row.errorMessage ?? '—' }}</td></tr></tbody></table></div>
        </section>
