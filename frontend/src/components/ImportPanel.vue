...
<template>
  <section class="import-panel simple-import-panel supplier-preview-panel" aria-label="Excel 导入面板">
    ...
    <div class="simple-import-body">
      ...
      <a v-if="type === 'ORDER'" data-test="download-order-template" class="order-template-download" href="/api/imports/templates/ORDER.xlsx" download>下载模板</a>
      ...
      <section v-else-if="previewBatch && importedRows === null && type === 'PRODUCT'" class="product-preview" aria-label="产品全量替换预览">
        ...
        <div class="supplier-preview-table-wrap"><table class="product-preview-table"><thead><tr><th>状态</th><th>来源</th><th>来源编号</th><th>计算编号</th><th>型号</th><th>产品配置</th><th>供应商</th><th>含税价</th><th>错误</th><th>冲突决定</th></tr></thead><tbody><tr v-for="row in previewBatch.rows" :key="row.id" data-test="preview-row"><td><span :class="['supplier-preview-status', row.status.toLowerCase()]">{{ row.status === 'VALID' ? '有效' : row.status === 'ERROR' ? '错误' : '忽略' }}</span></td><td class="compact-cell" :title="`${row.sheetName} · 第 ${row.rowNumber} 行`">{{ row.sheetName }} · 第 {{ row.rowNumber }} 行</td><td class="compact-cell" :title="value(row, 'sourceProductCode')">{{ value(row, 'sourceProductCode') }}</td><td class="compact-cell code-cell" :title="value(row, 'productCode')">{{ value(row, 'productCode') }}</td><td class="compact-cell" :data-test="`product-model-${row.id}`" :title="value(row, 'model')">{{ value(row, 'model') }}</td><td class="compact-cell" :title="value(row, 'productConfiguration')">{{ value(row, 'productConfiguration') }}</td><td class="compact-cell" :title="value(row, 'supplierName')">{{ value(row, 'supplierName') }}</td><td class="compact-cell" :title="value(row, 'supplierTaxPrice')">{{ value(row, 'supplierTaxPrice') }}</td><td class="compact-cell product-error-cell" :data-test="`product-row-error-${row.id}`" :title="row.errorMessage ?? ''">{{ row.errorMessage ?? '—' }}</td><td><template v-if="row.data._conflictGroup"><label class="decision-control"><input :data-test="`product-conflict-keep-${row.id}`" type="radio" :name="`product-${row.data._conflictGroup}`" :checked="productActions[row.id] === 'KEEP'" :disabled="row.status !== 'VALID' || busy" @change="chooseProductKeep(productConflictGroups.find(group => group.code === String(row.data._conflictGroup))?.rows ?? [], row.id)">保留</label><span v-if="productActions[row.id] === 'SKIP'" class="skip-state">跳过</span><span v-else-if="!productActions[row.id]" class="unresolved-state">未选择</span></template><span v-else>自动保留</span></td></tr><tr v-if="previewBatch.rows.length === 0"><td colspan="10">没有可预览的产品数据</td></tr></tbody></table></div>
