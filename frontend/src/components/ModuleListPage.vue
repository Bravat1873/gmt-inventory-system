...
const emit = defineEmits<{ action: []; manual: []; edit: [row: Record<string, unknown>]; workflow: [row: Record<string, unknown>]; shipment: [row: Record<string, unknown>]; details: [row: Record<string, unknown>]; receipt: [row: Record<string, unknown>]; payment: [row: Record<string, unknown>]; message: [text: string, kind?: 'success' | 'error'] }>()
const keyword = ref('')
...
function text(value: unknown, field?: string) {
  ...
  const statuses: Record<string, string> = { DRAFT: '草稿', PENDING_CUSTOMER_PAYMENT: '确认订单并分配库存', WAITING_STOCK: '等待齐货', READY_TO_SHIP: '等待发货', SHIPPED: '已发货', PENDING_SUPPLIER_PAYMENT: '待登记付款', EXECUTING: '采购执行中', RECEIVED: '已入库', RECEIVABLE: '收', PAYABLE: '付' }
  return statuses[String(value)] ?? String(value)
}
function shipmentCompleted(row: Record<string, unknown>) { return String(row.status) === 'SHIPPED' }
        ...
        <table :style="{ minWidth: `${tableMinWidth}px` }">
          ...
          <tbody>
            ...
            <tr v-for="row in data.items" v-else :key="`${String(row.recordType ?? module.key)}-${String(row.id)}`">
              <td v-for="field in module.fields" :key="field" :title="text(row[field], field)"><span v-if="module.key === 'order' && field === 'status'" class="order-shipment-status"><i class="shipment-status-dot" :class="shipmentCompleted(row) ? 'complete' : 'incomplete'"></i><span class="cell-content">{{ text(row[field], field) }}</span></span><span v-else-if="module.key === 'finance' && field === 'businessType'" data-test="finance-direction" class="finance-direction" :class="isReceivable(row) ? 'receivable' : 'payable'" :aria-label="isReceivable(row) ? '收款' : '付款'"><i aria-hidden="true"></i><span class="cell-content">{{ text(row[field], field) }}</span></span><span v-else class="cell-content">{{ text(row[field], field) }}</span></td>
              <td class="row-actions">
                ...
                <button v-if="module.key === 'purchase' && row.status === 'PENDING_SUPPLIER_PAYMENT'" data-test="purchase-payment" @click="emit('payment', row)">登记付款</button>
                <button v-if="module.key === 'purchase' && row.status !== 'PENDING_SUPPLIER_PAYMENT'" @click="emit('workflow', row)">办理</button>
              </td>
            </tr>
          </tbody>
        </table>
