            ...
            <tr v-for="row in data.items" v-else :key="`${String(row.recordType ?? module.key)}-${String(row.id)}`">
              ...
              <td class="row-actions"><div class="row-actions-content" :class="{ 'finance-actions': module.key === 'finance' }">
                ...
                <button v-if="module.key === 'customer'" data-test="customer-funds" @click="emit('funds', row)">资金</button>
                  <button v-if="canEditRow() && (['customer', 'user', 'product', 'supplier', 'inventory'].includes(module.key) || (module.key === 'order' && ['DRAFT', 'PENDING_CUSTOMER_PAYMENT', 'READY_TO_SHIP', 'WAITING_STOCK', 'SHIPPED'].includes(String(row.status))) || (module.key === 'purchase' && row.recordType === 'PURCHASE' && (Number(row.manualEntry ?? 0) === 0 || (Number(row.receivedQuantity ?? 0) === 0 && Number(row.paidAmount ?? 0) === 0))))" @click="emit('edit', row)">修改</button>
