    ...
    <section class="dialog-card order-dialog" role="dialog" aria-modal="true">
      ...
      <form novalidate @submit.prevent="save">
        ...
        <section class="order-section">
          ...
          <div class="order-lines-scroll" data-test="order-lines-scroll"><div class="order-lines"><div v-for="(line, index) in form.items" :key="index" class="order-line order-line-full"><span class="line-number">{{ index + 1 }}</span><label :class="{ 'field-invalid': lineError(index, 'sku') }"><span>物料编号 <small v-if="lineError(index, 'sku')" :data-test="`sku-error-${index}`" class="field-error">请选择物料</small></span><FuzzyPicker :data-test="`order-sku-picker-${index}`" v-model="line.skuId" :options="skuOptions" :placeholder="skus.length ? '输入物料编号、型号或名称搜索' : '暂无可选物料，请先导入产品或库存'" :disabled="saving || !skus.length" empty-text="没有匹配的物料" @update:model-value="value => selectSku(index, value)" /></label><div class="order-product-image-cell"><span>图片</span><img v-if="imageUrlFor(line)" :data-test="`order-product-image-${index}`" :src="imageUrlFor(line)" :alt="`${skuFor(line)?.productName || skuFor(line)?.skuCode || '产品'}主图`" @error="markImageFailed(line)"><div v-else :data-test="`order-image-placeholder-${index}`" class="order-image-placeholder">暂无图片</div></div><div :data-test="`order-inventory-${index}`" class="order-inventory-cell"><span>库存</span><strong>实际 {{ skuFor(line)?.actualQuantity ?? 0 }}</strong><strong :class="{ negative: (skuFor(line)?.availableQuantity ?? 0) < 0 }">可用 {{ skuFor(line)?.availableQuantity ?? 0 }}</strong></div><label><span>物料名称</span><input :value="skuFor(line)?.productName ?? ''" disabled></label><label><span>规格型号</span><input :value="[skuFor(line)?.model, skuFor(line)?.configuration].filter(Boolean).join(' / ')" disabled></label><label><span>单位</span><input :value="skuFor(line)?.unit ?? ''" disabled></label><label :class="{ 'field-invalid': lineError(index, 'quantity') }"><span>订单数量 <small v-if="lineError(index, 'quantity')" :data-test="`quantity-error-${index}`" class="field-error">数量须大于 0</small></span><input v-model.number="line.quantity" type="number" min="1"></label><label><span>已发货数量</span><input :value="line.shippedQuantity ?? 0" disabled></label><label><span>剩余数量</span><input :value="line.remainingQuantity ?? line.quantity" disabled></label><label :class="{ 'field-invalid': lineError(index, 'price') }"><span>含税单价 <small v-if="lineError(index, 'price')" :data-test="`price-error-${index}`" class="field-error">单价不可为负</small></span><input v-model.number="line.salePrice" type="number" min="0" step="0.01"></label><button :data-test="`remove-order-line-${index}`" type="button" @click="remove(index)">删除</button></div></div></div>
        </section>
