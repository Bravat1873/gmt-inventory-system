...
it('displays order refunds and the recorded adjustment reason in the fund ledger', async () => {
  vi.mocked(api.loadCustomerFundLedger).mockResolvedValueOnce([{ id: 99, entryType: 'ORDER_REFUND', direction: 'IN', amount: 20, balanceAfter: 520, sourceNo: 'DD-EDIT', reason: '订单修改：已收金额 100 → 80；订单降价', operatedAt: '2026-09-20T11:00:00' }])
  const wrapper = mount(CustomerFundsDialog, { props: { customer: { id: 7, customerName: '测试客户' }, currentUserRole: 'FINANCE' } })
  await flushPromises()
  await wrapper.findAll('.fund-tabs button')[1].trigger('click')
  expect(wrapper.text()).toContain('订单收款退回')
  expect(wrapper.text()).toContain('订单降价')
