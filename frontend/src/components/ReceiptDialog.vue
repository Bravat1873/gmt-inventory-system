...
const form = reactive({ amount: 0, paymentMethod: '银行转账', invoiceNo: '', receivedAt: new Date().toISOString().slice(0, 10) })
...
async function save() {
  ...
  if (value < 0 || (value === 0 && !form.invoiceNo.trim())) { error.value = '请填写收款金额或发票号码'; return }
  ...
  try {
    await postAction(`/api/finance/orders/${Number(props.order.id)}/receipt`, { amount: value, paymentMethod: form.paymentMethod.trim(), invoiceNo: form.invoiceNo.trim() || undefined, receivedAt: form.receivedAt })
    ...
    <section class="dialog-card receipt-dialog" role="dialog" aria-modal="true" aria-labelledby="receipt-title">
      ...
      <form novalidate @submit.prevent="save">
        ...
        <div class="form-grid">
          ...
          <label><span>本次收款金额</span><input data-test="receipt-amount" v-model.number="form.amount" type="number" min="0" :max="outstandingAmount" step="0.01" :disabled="saving"></label>
          ...
          <label><span>发票号码</span><input data-test="receipt-invoice-no" v-model="form.invoiceNo" :disabled="saving"></label>
          <label><span>登记后未收金额</span><input :value="money(afterReceiptAmount)" disabled></label>
        </div>
        <p v-if="outstandingAmount <= 0" class="receipt-complete">该订单已结清；可补录发票号码。</p>
        <p v-if="error" class="form-error" role="alert">{{ error }}</p>
        <footer><button type="button" class="secondary-action" :disabled="saving" @click="close">取消操作</button><button class="primary-action" :disabled="saving">{{ saving ? '正在保存…' : '确认登记' }}</button></footer>
      </form>
    </section>
  </div>
</template>
