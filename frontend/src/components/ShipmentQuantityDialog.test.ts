...
it('renders shipment items as product line cards with compact summary metrics', () => {
  ...
  expect(wrapper.get('[data-test="shipment-line"]').text()).toContain('已发货数量')
  expect(wrapper.get('[data-test="shipment-line"]').text()).toContain('本单锁定数量')
  expect(wrapper.get('[data-test="shipment-line"]').text()).toContain('本次发货数量')
  expect(wrapper.get('[data-test="shipment-line"]').text()).toContain('发货后未发货数量')
  expect(wrapper.get('[data-test="shipment-line"]').text()).not.toContain('累计已发')
  expect(wrapper.get('[data-test="shipment-line"]').text()).not.toContain('发货后剩余')
  expect(wrapper.get('[data-test="available-quantity"]').text()).toContain('未锁定库存数量')
