...
- 最终两份 SHA-256：`f5a0b04fd625622bcfb90210a371d60e3a0a918c4dbe6a6ee164a1039103f377`（一致）。

## Inspect / Render

- `订单导入!A1:U5`：确认 21 列固定顺序、两行同单正式订单、一行草稿订单、明显 DEMO 占位编码；第 5 行 21 个识别字段全部为空，空订单状态由后端默认正式订单。
...
- POI 结构检查：两个 sheet、21 列、示例分组/状态、冻结窗格、表格筛选、两个下拉列表、文本/日期/数量/金额格式、最多 101 行合理预留均通过；classpath 模板经 `SalesOrderExcelParser` 仅解析出 3 条示例数据。
...
- Review RED（后端）：新增 classpath 模板解析回归断言后，预期 3 条、实际 4 条；定位为第 5 行仅写入了“正式订单”。
- GREEN（后端）：brief 指定 Maven 命令，`ImportTemplateApiTest` 1/1 通过。
- Review GREEN：删除第 5 行订单状态物理值，保留后端对空状态默认“正式订单”的既有语义；同一聚焦测试 1/1 通过。
...
- 初始提交：`33f67b8 新增销售订单导入模板下载`。
- Review 修复提交：`修复订单模板空白行解析`；包含回归测试、builder、重新生成的 classpath 模板和本报告。
- `outputs/` 最终 XLSX 与 previews 保持未跟踪，不进入提交。
