...
public class FinanceController {
    ...
    @PostMapping("/{type}/invoices/{id}/review") public ApiResponse<Map<String,Object>> reviewInvoice(@PathVariable String type,@PathVariable long id,@RequestBody FinanceReviewRequest request){requireInvoiceWrite();return ApiResponse.ok(review.reviewInvoice(type,id,request));}
    ...
    @PostMapping("/{type}/{id}/invoices") public ApiResponse<Map<String,Object>> addInvoice(@PathVariable String type,@PathVariable long id,@RequestBody InvoiceRequest request){requireInvoiceWrite();return ApiResponse.ok(invoices.save(type,id,request));}
    @DeleteMapping("/{type}/{id}/invoices/{invoiceId}") public ApiResponse<Void> deleteInvoice(@PathVariable String type,@PathVariable long id,@PathVariable long invoiceId){requireInvoiceWrite();invoices.delete(type,id,invoiceId);return ApiResponse.ok(null);}
    @PutMapping("/{type}/{id}/invoice") public ApiResponse<Map<String,Object>> saveInvoice(@PathVariable String type,@PathVariable long id,@RequestBody InvoiceRequest request){requireInvoiceWrite();return ApiResponse.ok(invoices.save(type,id,request));}
    @DeleteMapping("/{type}/{id}/invoice") public ApiResponse<Void> deleteInvoice(@PathVariable String type,@PathVariable long id){requireInvoiceWrite();invoices.delete(type,id);return ApiResponse.ok(null);}
    private void requireFinanceWrite(){if(!CurrentUser.required().role().canWriteFinance())throw new SecurityException("无此操作权限");}
    private void requireInvoiceWrite(){if(!CurrentUser.required().role().canMaintainInvoices())throw new SecurityException("无发票维护权限");}
