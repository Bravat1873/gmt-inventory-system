...
public class FinanceReviewService {
    ...
    public Map<String,Object> reviewSummary(String type,long id) {
        ...
        BigDecimal invoiceTotal=zero(jdbc.queryForObject("SELECT COALESCE(SUM(COALESCE(confirmed_amount,tax_inclusive_amount)),0) FROM "+invoiceTable+" WHERE "+invoiceKey+"=? AND COALESCE(review_status,'APPROVED')='APPROVED'",BigDecimal.class,id));
    ...
    private Map<String,Object> findInvoiceForUpdate(String table,long id){try{return jdbc.queryForMap("SELECT id,invoice_no,tax_inclusive_amount,review_status FROM "+table+" WHERE id=? FOR UPDATE",id);}catch(EmptyResultDataAccessException e){throw new IllegalArgumentException("发票记录不存在");}}
    private static BigDecimal confirmedAmount(BigDecimal original,BigDecimal requested){if(requested==null)return zero(original);if(requested.signum()<=0)throw new IllegalArgumentException("财务确认金额必须大于 0");return original!=null&&original.signum()<0?requested.negate():requested;}
    private static String invoiceTable(String type){return "SALES".equals(normalizeType(type))?"sales_invoice":"purchase_invoice";}
    ...
    private static long num(Object v){return ((Number)v).longValue();} private static BigDecimal decimal(Object v){return v==null?null:v instanceof BigDecimal b?b:new BigDecimal(String.valueOf(v));} private static BigDecimal zero(BigDecimal v){return v==null?BigDecimal.ZERO:v;} private static String clean(String v){return v==null||v.isBlank()?null:v.trim();}
}
