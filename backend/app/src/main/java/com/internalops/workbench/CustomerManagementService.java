...
public class CustomerManagementService {
    ...
    public Map<String,Object> detail(long id) {
        ...
        List<Map<String,Object>> contracts = jdbc.queryForList("SELECT id,contract_no AS contractNo,start_date AS startDate,end_date AS endDate,remark FROM customer_contract WHERE customer_id=? AND enabled=TRUE ORDER BY start_date DESC,id DESC", id).stream().map(this::camel).map(LinkedHashMap::new).map(row -> {
            row.put("prices", jdbc.queryForList("SELECT p.sku_id AS skuId,p.sale_price AS salePrice,s.customer_part_number AS customerPartNumber,s.product_name AS productName FROM customer_contract_price p JOIN sku s ON s.id=p.sku_id WHERE p.contract_id=? ORDER BY s.customer_part_number,s.id", row.get("id")).stream().map(this::camel).toList());
