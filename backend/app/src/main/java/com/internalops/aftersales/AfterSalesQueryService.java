...
@Service public class AfterSalesQueryService {
 ...
 public Map<String,Object> refundSuggestion(long id){
  ...
  BigDecimal suggested=jdbc.queryForObject("SELECT COALESCE(SUM(r.received_quantity*COALESCE(r.return_unit_price,i.sale_price)),0) FROM after_sales_return_line r JOIN sales_order_item i ON i.id=r.sales_order_item_id WHERE r.after_sales_order_id=?",BigDecimal.class,id);
 ...
 public List<Map<String,Object>> orderLines(long orderId){return jdbc.queryForList("SELECT i.id sales_order_item_id,i.sku_id,i.sale_price,s.product_code,s.customer_part_number,s.product_name,s.model,s.configuration,s.unit,i.shipped_quantity-COALESCE((SELECT SUM(r.requested_quantity) FROM after_sales_return_line r JOIN after_sales_order a ON a.id=r.after_sales_order_id WHERE r.sales_order_item_id=i.id AND a.status<>'CANCELLED'),0) available_return_quantity FROM sales_order_item i JOIN sku s ON s.id=i.sku_id WHERE i.sales_order_id=? AND i.shipped_quantity>0",orderId).stream().map(this::camel).toList();}
