...
public class AuthService {
    ...
    public Session login(String username, String password) {
        List<User> users = jdbc.query("SELECT id,username,password_hash,display_name FROM sys_user WHERE username=? AND enabled=TRUE",
                (rs, i) -> new User(rs.getLong(1),rs.getString(2),rs.getString(3),rs.getString(4)), username.trim());
        ...
        return new Session(token, new CurrentUser(user.id(),user.username(),user.displayName()));
    }
    public CurrentUser current(String token) {
        if (token==null || token.isBlank()) return null;
        List<CurrentUser> users=jdbc.query("SELECT u.id,u.username,u.display_name FROM user_session s JOIN sys_user u ON u.id=s.user_id WHERE s.token_hash=? AND s.revoked_at IS NULL AND s.expires_at>CURRENT_TIMESTAMP AND u.enabled=TRUE",(rs,i)->new CurrentUser(rs.getLong(1),rs.getString(2),rs.getString(3)),hash(token));
    ...
    private record User(long id,String username,String hash,String displayName) {}
