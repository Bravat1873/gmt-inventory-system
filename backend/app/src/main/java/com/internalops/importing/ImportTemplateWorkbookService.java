...
final class ImportTemplateWorkbookService {
    ...
    private List<String> headers(ImportType type) {
        return switch (type) {
            ...
            case PRODUCT -> List.of("产品分类", "物料类型", "客户料号", "产品编号（不用写）", "品牌", "系列", "物料颜色", "锁体类型", "联网方式", "销售渠道", "运营主体", "语言", "编码后缀", "型号（系列号+第几代）", "物料规格（不用写，会自动生成）", "产品配置", "销售最小起订量", "供应商名称", "供应商含税价", "实际库存数量", "已锁定数量", "在途数量", "库存供应商", "库存备注");
    ...
    private void writeProductRulesSheet(org.apache.poi.ss.usermodel.Sheet sheet) {
        ...
        fields.createCell(0).setCellValue("产品分类、物料类型、客户料号、品牌、系列、物料颜色、锁体类型、联网方式、销售渠道、运营主体、语言、编码后缀、型号、产品配置、销售最小起订量、供应商名称、供应商含税价、实际库存数量、已锁定数量、在途数量、库存供应商、库存备注");
    }
}
