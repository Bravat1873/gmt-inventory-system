...
public class CustomerFundQueryService {
    ...
    private static final class Accumulator {
        ...
        void add(Map<String,Object> row){BigDecimal value=decimal(row.get("amount"));String type=String.valueOf(row.get("entry_type"));String direction=String.valueOf(row.get("direction"));switch(type){case "CUSTOMER_DEPOSIT"->deposit=deposit.add("IN".equals(direction)?value:value.negate());case "ORDER_RECEIPT"->receipt=receipt.add(value);case "ORDER_REFUND"->receipt=receipt.subtract(value);case "AFTER_SALES_REFUND"->refund=refund.add(value);case "REVERSAL"->reversal=reversal.add("IN".equals(direction)?value:value.negate());default->{}}closing=decimal(row.get("balance_after"));}
