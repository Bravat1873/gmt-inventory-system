DROP TABLE IF EXISTS sales_order_item;
DROP TABLE IF EXISTS sales_order;
DROP TABLE IF EXISTS warehouse;
DROP TABLE IF EXISTS inventory_transaction;
DROP TABLE IF EXISTS inventory_locked_allocation;
DROP TABLE IF EXISTS inventory_balance;
DROP TABLE IF EXISTS sku;

CREATE TABLE sku (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    product_code VARCHAR(80),
    customer_part_number VARCHAR(80) NOT NULL,
    model VARCHAR(100),
    product_type VARCHAR(30),
    product_configuration VARCHAR(500),
    configuration VARCHAR(1000),
    product_version VARCHAR(80),
    color VARCHAR(80),
    lock_body VARCHAR(80),
    unit VARCHAR(20)
);
CREATE TABLE warehouse (
    id BIGINT PRIMARY KEY,
    warehouse_code VARCHAR(40),
    warehouse_name VARCHAR(100),
    is_default BOOLEAN NOT NULL,
    enabled BOOLEAN NOT NULL
);
CREATE TABLE sales_order (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    order_no VARCHAR(50),
    status VARCHAR(40)
);
CREATE TABLE sales_order_item (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    sales_order_id BIGINT NOT NULL,
    sku_id BIGINT NOT NULL,
    quantity INT NOT NULL,
    shipped_quantity INT NOT NULL DEFAULT 0
);CREATE TABLE inventory_balance (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    warehouse_id BIGINT NOT NULL,
    sku_id BIGINT NOT NULL,
    actual_quantity INT NOT NULL,
    locked_quantity INT NOT NULL,
    in_transit_quantity INT NOT NULL,
    source_supplier_name VARCHAR(200),
    inventory_remark VARCHAR(500),
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    version INT NOT NULL DEFAULT 0
);
CREATE TABLE inventory_locked_allocation (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    inventory_balance_id BIGINT NOT NULL,
    lock_source VARCHAR(30) NOT NULL,
    quantity INT NOT NULL
);
CREATE TABLE inventory_transaction (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    warehouse_id BIGINT NOT NULL,
    sku_id BIGINT NOT NULL,
    transaction_type VARCHAR(40) NOT NULL,
    business_type VARCHAR(40) NOT NULL,
    business_no VARCHAR(80) NOT NULL,
    actual_delta INT NOT NULL,
    locked_delta INT NOT NULL,
    transit_delta INT NOT NULL,
    actual_before INT NOT NULL,
    actual_after INT NOT NULL,
    locked_before INT NOT NULL,
    locked_after INT NOT NULL,
    transit_before INT NOT NULL,
    transit_after INT NOT NULL,
    operated_at TIMESTAMP NOT NULL
);

INSERT INTO sku(id,product_code,customer_part_number,model,product_type,product_configuration,configuration,product_version,color,lock_body,unit)
VALUES(1,'SXSEL_P90','SKU-LOCK','P90','SMART_LOCK','可视对讲 + 指纹','智能锁','V1','宇宙黑','6068','套');
INSERT INTO warehouse VALUES(1,'MAIN','Main',TRUE,TRUE);
INSERT INTO sales_order(order_no,status) VALUES('SO-DEMAND','WAITING_STOCK');
INSERT INTO sales_order_item(sales_order_id,sku_id,quantity,shipped_quantity) SELECT id,1,8,2 FROM sales_order WHERE order_no='SO-DEMAND';
INSERT INTO inventory_balance(id,warehouse_id,sku_id,actual_quantity,locked_quantity,in_transit_quantity,source_supplier_name,inventory_remark)
VALUES(1,1,1,20,15,5,'易达胜','库存备注');
INSERT INTO inventory_balance(id,warehouse_id,sku_id,actual_quantity,locked_quantity,in_transit_quantity,source_supplier_name,inventory_remark,updated_at)
VALUES(2,1,1,0,0,0,'零库存','',TIMESTAMP '2000-01-01 00:00:00'),
      (3,1,1,21,0,0,'流水不足','',TIMESTAMP '2000-01-01 00:00:00');
INSERT INTO inventory_locked_allocation(inventory_balance_id,lock_source,quantity) VALUES
(1,'铭爱钧乔',1),(1,'博乐龙米',2),(1,'老挝',3),(1,'贝朗',4),(1,'马来西亚',5);
INSERT INTO inventory_transaction(warehouse_id,sku_id,transaction_type,business_type,business_no,actual_delta,locked_delta,transit_delta,actual_before,actual_after,locked_before,locked_after,transit_before,transit_after,operated_at) VALUES
(1,1,'EXCEL_OUTBOUND','EXCEL_IMPORT_HISTORY','B7:T:0622出库',7,0,0,20,20,15,15,5,5,TIMESTAMP '2026-06-22 00:00:00'),
(1,1,'EXCEL_INBOUND','EXCEL_IMPORT_HISTORY','B7:U:0804入库',9,0,0,20,20,15,15,5,5,TIMESTAMP '2026-08-04 00:00:00');

INSERT INTO inventory_transaction(warehouse_id,sku_id,transaction_type,business_type,business_no,actual_delta,locked_delta,transit_delta,actual_before,actual_after,locked_before,locked_after,transit_before,transit_after,operated_at)
VALUES(1,1,'PURCHASE_RECEIPT','PURCHASE_ORDER','PO-20260601',18,0,0,0,18,0,0,0,0,TIMESTAMP '2026-06-01 00:00:00'),
      (1,1,'INITIAL_IMPORT','EXCEL_IMPORT','7',20,0,0,0,20,0,0,0,0,TIMESTAMP '2026-08-07 12:00:00');

ALTER TABLE sku ADD COLUMN sales_minimum_order_quantity INT NOT NULL DEFAULT 1;
