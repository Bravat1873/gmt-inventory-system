...
class ProductReplaceImportApiTest {
    ...
    @Test
    void eachRoleCanCommitIncrementalProductImports() throws Exception {
        ...
        mvc.perform(post("/api/imports/{batchId}/commit", financeBatch).cookie(login("finance"))
                ...
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.result.mode").value("PRODUCT_INCREMENTAL"));
    ...
    private long batch(String hash) {
        ...
        data.put("customerPartNumber", "P90-TEST-" + hash);
